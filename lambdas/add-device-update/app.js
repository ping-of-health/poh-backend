...
function createDoc (db, json, callback) {
  db.collection('device_updates').insertOne( json, function(err, result) {
      if(err!=null) {
          console.error("an error occurred in createDoc", err);
          callback(null, JSON.stringify(err));
      }
      else {
        console.log("Kudos! You just created an entry into the device_updates collection with id: " + result.insertedId);
